...
public function __construct($input = false) {
   ...
   protected function _verifyInput() {
    ...
    if ($password1) {
        if ($password1 != trim($_POST['password1'])) {
            $error = true;
  ...
  public static function deleteRecord($id) {
    ...
    if ($result = $connection->query($query)) {
        $return = array('', 'Contact Record succesfully delete.', '');
        return $return;
    } else {
...
public function getAccess_DropDown() {
    // set up fi rst option for selection if none selected
    $option_selected = '';
    if (!$this->access) {
